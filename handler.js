      ...
      if (chat.welcome && !chat?.isBanned) {
        ...
        for (const user of participants) {
          ...
          try {
	        ...
	        (chat.sStock || conn.stock || 'NO TIENES NINGUN STOCK DE VENTAS CONFIGURADO').replace('@user', '@' + user.split('@')[0])
		  (chat.sPagos || conn.pagos || 'NO TIENES NINGUN METODO DE PAGO CONFIGURADO').replace('@user', '@' + user.split('@')[0])
		    (chat.sReglas || conn.reglas || 'NO TIENES NINGUN TIPO DE REGLAS CONFIGURADO').replace('@user', '@' + user.split('@')[0]);
