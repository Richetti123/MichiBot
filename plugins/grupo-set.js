let handler = async (m, { conn, command, args, text }) => {
  ...
  // Para consultas dinámicas, solo responde si el mensaje empieza con '.' (u otro prefijo)
  if (!text || !text.startsWith('.')) return
  let withoutPrefix = text.slice(1).trim()
  if (!withoutPrefix) return

  let [type, name, ...rest] = withoutPrefix.split(' ')
  if (!type) return
  ...
  if (entry.type === 'image') {
    try {
      ...
      await conn.sendFile(m.chat, buffer, `${type}-${name}.jpg`, `📌 *${type.toUpperCase()} - ${name}*`, m)
    } catch {
...
handler.command = ['set', /^[.]?\w+$/i] // 'set' para configurar, comandos que empiecen opcionalmente con '.' y palabra
handler.group = true
handler.admin = true
